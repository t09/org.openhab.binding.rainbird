...
public class RainbirdPayloadCoder {
    ...
    public RainbirdPayloadCoder(@Nullable String password) {
    ...
    public byte[] encode(Map<String, @Nullable Object> payload) throws IOException {
    ...
    public Map<String, @Nullable Object> decode(byte[] payload) throws IOException {
        String json;
        byte @Nullable [] key = sessionKey;
        if (key == null) {
            json = new String(payload, StandardCharsets.UTF_8);
        } else {
            json = decrypt(payload, key);
        }
